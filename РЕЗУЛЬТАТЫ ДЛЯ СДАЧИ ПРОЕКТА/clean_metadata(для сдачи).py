...
def main():
    print(f"Читаю исхо файл: {RAW_METADATA_PATH}")
    df = pd.read_csv(RAW_METADATA_PATH)

    print(f"Исходное колво строк: {len(df)}")
    ...
    print(f"Пропуски в age до очистки: {before_age_clean}, тут после: {after_age_clean}")

    # Заполняем все пропуски в поле sex(пол)
    df["sex"] = df["sex"].apply(normalize_sex)

    # Вычисляем медианну возраста (по непустым)
    age_median = df["age"].median()
    print(f"Медианный возраст: {age_median}")

    # Заполняем пропуски возраста медианной
    ...
    # Убеждаемся, что директория действительно существует(можнопропустить)
